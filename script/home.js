...
// Helper function to get todos for the logged-in user
function getTodos() {
  if (!username) return []; // If no user is logged in, return an empty array
  const todos = localStorage.getItem(`todos_${username}`); // Use a unique key for each user
  return todos ? JSON.parse(todos) : [];
}

// Helper function to save todos for the logged-in user
function saveTodos(todos) {
  if (username) {
    localStorage.setItem(`todos_${username}`, JSON.stringify(todos)); // Store todos with a unique key for the user
  }
}
...
function renderTodos() {
  ...
  const todos = getTodos(); // Get todos for the current user
  todos.forEach((todo) => {
...
// Initial render of todos for the logged-in user
renderTodos();
